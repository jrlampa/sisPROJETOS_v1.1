...
class AIAssistantGUI(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Header
        self.header = ctk.CTkLabel(self, text="Assistente Técnico IA (Groq)", font=DesignSystem.FONT_SUBHEAD, text_color=DesignSystem.TEXT_MAIN)
        self.header.grid(row=0, column=0, pady=20)
        
        # Chat area
        self.chat_area = ctk.CTkTextbox(self, state="disabled", wrap="word", font=DesignSystem.FONT_BODY, fg_color="white", text_color=DesignSystem.TEXT_MAIN, border_width=1, border_color="#E2E8F0")
        ...
        self.entry_msg = ctk.CTkEntry(self.input_frame, placeholder_text="Digite sua dúvida técnica aqui...", **DesignSystem.get_entry_style())
        self.entry_msg.grid(row=0, column=0, sticky="ew", padx=(0, 10))
        self.entry_msg.bind("<Return>", lambda e: self.send_message())
        
        self.btn_send = ctk.CTkButton(self.input_frame, text="Enviar", width=100, command=self.send_message, **DesignSystem.get_button_style("primary"))
        ...
        ctk.CTkButton(self.ctrl_frame, text="Limpar Chat", command=self.clear_chat, **DesignSystem.get_button_style("gray")).pack(side="left")
        ctk.CTkButton(self.ctrl_frame, text="Voltar ao Menu", command=lambda: self.controller.show_frame("Menu"), **DesignSystem.get_button_style("secondary")).pack(side="right")
