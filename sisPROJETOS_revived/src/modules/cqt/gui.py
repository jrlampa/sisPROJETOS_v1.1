...
class CQTGUI(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.settings_bar = ctk.CTkFrame(self, fg_color="#F8FAFC", corner_radius=12, border_width=1, border_color="#E2E8F0")
        self.settings_bar.pack(fill="x", padx=20, pady=(0, 10))

        ctk.CTkLabel(self.settings_bar, text="Trafo (kVA):", font=DesignSystem.FONT_BODY).pack(side="left", padx=(15, 5), pady=10)
        ...
        self.cmb_class = ctk.CTkComboBox(self.settings_bar, values=["A", "B", "C", "D"], width=80, **DesignSystem.get_entry_style())
        self.cmb_class.set("B")
        self.cmb_class.pack(side="left", padx=5)

        # Action Buttons
        self.btn_calc = ctk.CTkButton(self.settings_bar, text="Calcular", width=100, command=self.calculate, **DesignSystem.get_button_style("primary"))
        self.btn_calc.pack(side="right", padx=15)
        
        self.btn_add = ctk.CTkButton(self.settings_bar, text="+ Trecho", width=100, command=self.add_row, **DesignSystem.get_button_style("success"))
        self.btn_add.pack(side="right", padx=5)

        # Table Area
        self.table_side = ctk.CTkFrame(self, fg_color="white", corner_radius=15, border_width=1, border_color="#E2E8F0")
        ...
        for i, (h, w) in enumerate(zip(headers, self.header_widths)):
            lbl = ctk.CTkLabel(self.header_row, text=h, font=DesignSystem.FONT_BODY, width=w, text_color=DesignSystem.TEXT_DIM)
        ...
        self.lbl_summary = ctk.CTkLabel(self.footer, text="Resumo: Aguardando cálculo...", font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_DIM)
        self.lbl_summary.pack(side="left")

        self.btn_export = ctk.CTkButton(self.footer, text="Excel", width=100, command=self.export_excel, **DesignSystem.get_button_style("secondary"))
        self.btn_export.pack(side="right", padx=5)

        self.btn_back = ctk.CTkButton(self.footer, text="Menu", width=100, command=lambda: self.controller.show_frame("Menu"), **DesignSystem.get_button_style("gray"))
    ...
    def add_row(self, values=None):
        ...
        for i, (f, w) in enumerate(zip(fields, self.header_widths)):
            ...
            if values and i < len(values):
                if f == "cabo": ent.set(values[i])
                else: ent.insert(0, values[i])
                
            row_widgets[f] = ent
            
        # Add Delete Button
        if row_idx > 1: # Don't delete TRAFO
            btn_del = ctk.CTkButton(self.table_container, text="X", width=30, fg_color="red", command=lambda r=row_widgets: self.remove_row(r))
    ...
    def calculate(self):
        ...
        if res["success"]:
            summary = res["summary"]
            txt = (f"CQT Máximo: {summary['max_cqt']:.2f}% | "
        ...
        if file_path:
            try:
                ...
                if "cqt" in self.controller.project_context:
                    ...
                    df["CQT_Acumulado_%"] = df["ponto"].apply(lambda x: res.get(x.upper(), {}).get("cqt_accumulated", 0))
                    df["Carga_Acumulada_kVA"] = df["ponto"].apply(lambda x: res.get(x.upper(), {}).get("accumulated", 0))
