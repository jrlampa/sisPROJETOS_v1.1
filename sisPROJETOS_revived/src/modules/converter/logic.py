    ...
    def convert_to_utm(self, placemarks):
        ...
        for idx, p in enumerate(placemarks):
            try:
                ...
                if coords is None and hasattr(p.geometry, '__geo_interface__'):
                    try:
                        ...
                        if geo.get('type') == 'Point' and 'coordinates' in geo:
                            ...
                            coords = [(float(coords_raw[0]), float(coords_raw[1]), float(coords_raw[2]) if len(coords_raw) > 2 else 0)]
                            geom_type = 'Point'
                            ...
                            for coord in coords_raw:
                                if isinstance(coord, (tuple, list)) and len(coord) >= 2:
                                    coords.append((float(coord[0]), float(coord[1]), float(coord[2]) if len(coord) > 2 else 0))
                ...
                if not coords:
                    skipped.append(f"Placemark {idx+1} ('{getattr(p, 'name', 'Unnamed')}'): Could not extract coordinates")
                    continue
                ...
                if not (-180 <= lon <= 180 and -90 <= lat <= 90):
                    skipped.append(f"Placemark {idx+1} ('{getattr(p, 'name', 'Unnamed')}'): Invalid coordinates ({lon}, {lat})")
                    continue
    ...
    def save_to_dxf(self, df, filepath):
        ...
        for name, group in grouped:
            ...
            if len(group) == 1:
                ...
                # Line/Polyline
                points = [(float(e), float(n), float(z)) 
