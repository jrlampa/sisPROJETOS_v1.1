...
class CatenaryGUI(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(self.controls_frame, text="Parâmetros", font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_DIM).pack(pady=10)
        
        # Conductor Selection
        ctk.CTkLabel(self.controls_frame, text="Condutor:", font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_MAIN).pack(anchor="w", padx=15)
        self.cmb_conductor = ctk.CTkComboBox(self.controls_frame, values=["Carregando..."], command=self.on_conductor_change, **DesignSystem.get_entry_style())
        self.cmb_conductor.pack(fill="x", padx=15, pady=5)
        
        # Numeric Inputs
        self.inputs = {}
        for label, key in [("Vão (m)", "span"), ("Altura A (m)", "ha"), ("Altura B (m)", "hb"), ("Tração (daN)", "tension")]:
            ctk.CTkLabel(self.controls_frame, text=label, font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_MAIN).pack(anchor="w", padx=15)
        ...
        self.lbl_result = ctk.CTkLabel(self.controls_frame, text="Flecha: -", font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_DIM)
        ...
        # --- Plot Panel ---
        self.plot_panel = ctk.CTkFrame(self.main_container, fg_color="white", corner_radius=15, border_width=1, border_color="#E2E8F0")
    ...
    def plot(self, data):
        ...
        self.ax.scatter([0, data["x_vals"][-1]], [data["y_vals"][0], data["y_vals"][-1]], color='red', label="Suportes") # Supports
