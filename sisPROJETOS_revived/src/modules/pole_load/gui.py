...
class PoleLoadGUI(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.right_frame = ctk.CTkFrame(self, fg_color="white", corner_radius=15, border_width=1, border_color="#E2E8F0")
        self.right_frame.grid(row=0, column=1, sticky="nsew", padx=10, pady=10)
        
        # Controls Group
        self.params_frame = ctk.CTkFrame(self.left_frame, fg_color="#F8FAFC", corner_radius=15, border_width=1, border_color="#E2E8F0")
        self.params_frame.pack(fill="x", padx=10, pady=10)
        
        ctk.CTkLabel(self.params_frame, text="Parâmetros Gerais", font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_DIM).pack(pady=10)
        
        self.create_label_and_cmb(self.params_frame, "Concessionária:", self.logic.DADOS_CONCESSIONARIAS.keys(), self.update_options, "cmb_concess")
        ...
        ctk.CTkLabel(self.cables_header, text="Condutores no Poste", font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_MAIN).pack(side="left")
        self.btn_add = ctk.CTkButton(self.cables_header, text="+ Cabo", width=80, height=30, command=self.add_cable_row, **DesignSystem.get_button_style("success"))
        ...
        ctk.CTkButton(self.actions_frame, text="Calcular Esforço Resultante", command=self.calculate, **DesignSystem.get_button_style("primary")).pack(fill="x", pady=5)
        
        self.btn_suggest = ctk.CTkButton(self.actions_frame, text="Sugerir Melhor Poste (IA)", command=self.suggest_best_pole, state="disabled", **DesignSystem.get_button_style("purple"))
        self.btn_suggest.pack(fill="x", pady=2)
        
        self.btn_report = ctk.CTkButton(self.actions_frame, text="Gerar Relatório PDF", command=self.generate_pdf, state="disabled", **DesignSystem.get_button_style("secondary"))
        ...
        ctk.CTkLabel(self.pole_check_frame, text="Verificação de Segurança", font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_DIM).pack(pady=5)
        
        self.create_label_and_cmb(self.pole_check_frame, "Poste:", self.logic.DADOS_POSTES_NOMINAL.keys(), self.update_pole_specs, "cmb_pole_type")
        self.create_label_and_cmb(self.pole_check_frame, "Capacidade:", [], None, "cmb_pole_spec")
        
        # Back
        ctk.CTkButton(self.left_frame, text="Voltar ao Menu", command=lambda: self.controller.show_frame("Menu"), **DesignSystem.get_button_style("gray")).pack(pady=20)
        ...
        self.lbl_result = ctk.CTkLabel(self.right_frame, text="Diagrama de Vetores de Trações", font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_DIM)
    ...
    def create_label_and_cmb(self, parent, label_text, values, cmd, attr_name):
        ctk.CTkLabel(parent, text=label_text, font=DesignSystem.FONT_BODY, text_color=DesignSystem.TEXT_MAIN).pack(anchor="w", padx=15)
    ...
    def update_pole_specs(self, choice=None):
        ...
        if specs: self.cmb_pole_spec.set(specs[0])

    def add_cable_row(self):
        row = ctk.CTkFrame(self.cables_container)
        row.pack(fill="x", pady=2)
        
        concess = self.logic.DADOS_CONCESSIONARIAS.get(self.cmb_concess.get(), list(self.logic.DADOS_CONCESSIONARIAS.values())[0])
        ...
        self.cables.append({
            'row': row, 'net': cmb_net, 'cond': cmb_cond, 'span': entry_span, 
            'ang': entry_angle, 'flecha': entry_flecha
        })
    ...
    def calculate(self):
        ...
        try:
            ...
            for c in self.cables:
                ...
                if not span_val or not ang_val: continue
            ...
            if res:
                 ...
                 self.lbl_result.configure(text=f"Força Resultante: {res['resultant_force']:.2f} daN\nÂngulo: {res['resultant_angle']:.2f}°\n\nLimite do Poste: {limit} daN\n{status}", text_color=color)
                 self.btn_report.configure(state="normal")
                 
        except ValueError:
            messagebox.showerror("Erro", "Verifique se os campos numéricos (Vão, Ângulo, Flecha) estão preenchidos corretamente.")
    ...
    def plot(self, data):
        ...
        # Resultant
        self.ax.arrow(0, 0, data['total_x'], data['total_y'], head_width=5, color='red', linewidth=2, label='Resultante')
